...
from reachy_mini_conversation_app.cascade.speech_output import split_into_sentences
...
def test_split_into_sentences_splits_on_punctuation() -> None:
    """Long multi-sentence text is split into separate chunks."""
    chunks = split_into_sentences("Hello there. How are you doing today? I am fine.")
    assert len(chunks) == 3
    assert chunks[0] == "Hello there."
    assert "".join(chunks).replace(" ", "") == "Hellothere.Howareyoudoingtoday?Iamfine."


def test_split_into_sentences_merges_short_fragments() -> None:
    """Fragments below min_length are merged so TTS never gets choppy inputs."""
    # Each fragment ("Hi.", "Yo.", "Ok.") is shorter than min_length and gets merged.
    assert split_into_sentences("Hi. Yo. Ok.") == ["Hi. Yo. Ok."]


def test_split_into_sentences_no_punctuation_returns_whole_text() -> None:
    """Text without sentence punctuation is returned as a single chunk."""
    assert split_into_sentences("just one phrase") == ["just one phrase"]
