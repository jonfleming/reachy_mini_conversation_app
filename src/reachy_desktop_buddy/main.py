...
from reachy_desktop_buddy.utils import (
    parse_args,
    setup_logger,
    ...
    if _import_warmup_thread is None:
        ...
        def _preload() -> None:
            try:
                ...
                import reachy_desktop_buddy.huggingface_realtime  # noqa: F401
            except Exception:
...
def main() -> None:
    ...
    if args.command == "tool-spaces":
        from reachy_desktop_buddy.tool_spaces import handle_tool_spaces_command
...
def run(
    ...
    if buddy_runtime.enabled:
        try:
            ...
            from reachy_desktop_buddy.config import config as app_config
...
class ReachyDesktopBuddy(ReachyMiniApp):  # type: ignore[misc]
    """Reachy Mini Apps entry point for Reachy Desktop Buddy."""
