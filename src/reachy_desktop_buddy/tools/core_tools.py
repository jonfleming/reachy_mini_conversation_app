...
from reachy_desktop_buddy.config import config, list_tool_module_names
from reachy_desktop_buddy.mcp_client import McpToolTimeoutError, McpToolInvocationError
from reachy_desktop_buddy.tool_spaces import build_remote_client, read_installed_tool_spaces
from reachy_desktop_buddy.profile_store import DEFAULT_PROFILE_NAME
from reachy_desktop_buddy.profile_toolsets import read_profile_tool_names
from reachy_desktop_buddy.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_desktop_buddy.mcp_client import RemoteMcpToolClient
    from reachy_desktop_buddy.tools.background_tool_manager import BackgroundToolManager
...
_EXTERNAL_TOOL_MODULE_NAMESPACE = "reachy_desktop_buddy._external_tools"
...
def _load_enabled_tools(tool_names: list[str], remote_tool_names: set[str]) -> List[type[Tool]]:
    ...
    for tool_name in tool_names:
        ...
        shared_module_path = f"reachy_desktop_buddy.tools.{tool_name}"
        try:
