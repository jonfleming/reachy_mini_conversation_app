...
class CascadeHandler(ConversationHandler):
    ...
    def __init__(
        ...
        # Streaming ASR: when the active provider supports it, we transcribe
        # incrementally as the user speaks instead of batch-transcribing at the end.
        self._streaming_asr: StreamingASRProvider | None = (
            self.asr if isinstance(self.asr, StreamingASRProvider) else None
        )
        ...
        while len(self._vad_buffer) >= VAD_CHUNK_SIZE:
            ...
            if event == VADEvent.SPEECH_STARTED:
                ...
                if self._streaming_asr is not None:
                    # Open the stream and feed the pre-roll the VAD buffered before
                    # the trigger so the first word isn't lost.
                    await self._streaming_asr.start_stream()
                    self._last_partial = ""
                    await self._feed_asr_chunk(np.concatenate(self._vad_sm.speech_chunks))
            elif event == VADEvent.SPEECH_ENDED:
                # Streaming providers already hold the audio; finalize via end_stream().
                wav = None
                if self._streaming_asr is None:
                    ...
                    wav = _pcm_to_wav(speech, SILERO_SAMPLE_RATE)
                self._turn_task = asyncio.create_task(self._run_turn(wav))
            elif self._streaming_asr is not None and self._vad_sm.state == VADState.RECORDING:
                await self._feed_asr_chunk(vad_chunk)
    ...
    async def _feed_asr_chunk(self, pcm: NDArray[np.int16]) -> None:
        """Stream PCM to the ASR and surface partial transcripts for live reactions."""
        assert self._streaming_asr is not None
        if pcm.size == 0:
            return
        await self._streaming_asr.send_audio_chunk(_pcm_to_wav(pcm, SILERO_SAMPLE_RATE))
        partial = await self._streaming_asr.get_partial_transcript()
        if partial and partial != self._last_partial:
            self._last_partial = partial
            logger.info("🎤 Partial: %s", partial)
            await self.transcript_manager.analyze_partial(partial)

    async def _run_turn(self, wav_bytes: bytes | None) -> None:
        """Transcribe, run the LLM/tool/TTS pipeline, then return VAD to listening.

        ``wav_bytes`` carries the full utterance for batch ASR providers; for
        streaming providers it is None and the transcript comes from end_stream().
        """
        ...
        async with self._processing_lock:
            try:
                ...
                if wav_bytes is None:
                    ...
                    transcript = await self._streaming_asr.end_stream()
                else:
                ...
                await self._emit_turn_items(result.turn_items)
    ...
    async def _emit_turn_items(self, turn_items: list[TurnItem]) -> None:
        """Push each displayable turn item to the Gradio chatbot transcript.

        Cascade is batch (not streamed), so items appear after the robot has
        spoken. Mirrors the AdditionalOutputs payloads used by the realtime backends.
        """
        for item in turn_items:
            if item.kind in ("speak", "assistant"):
                payload: Dict[str, Any] = {"role": "assistant", "content": item.text}
            elif item.kind == "tool":
                payload = {
                    "role": "assistant",
                ...
                image = Image.open(io.BytesIO(item.image_jpeg))
                payload = {"role": "assistant", "content": gr.Image(value=image)}
            else:
                logger.warning("Unknown turn item kind: %s", item.kind)
                continue
