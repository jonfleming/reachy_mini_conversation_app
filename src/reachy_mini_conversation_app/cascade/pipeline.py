...
def _log_prompt(messages: list[dict[str, Any]], tools: list[dict[str, Any]], system: str | None, depth: int) -> None:
    """Log a human-readable snapshot of the LLM request at debug level."""
    import datetime
...
async def execute_tool_calls(
    ...
    """Execute tool calls and handle the camera (image) and speak tools specially."""
    ...
    for tool_call in tool_calls:
        ...
        try:
            ...
            # Execute tool. "speak" is cascade-specific (not in the shared tool registry):
            ...
            # The camera tool returns a raw image ({"b64_im": ...}) when no separate
            # vision processor is configured, so the (multimodal) LLM can see it.
            # Strip the heavy blob from history and re-inject the image as a user
            # message below. A text answer ({"image_description": ...}) is handled
            ...
            if tool_name == "camera" and "b64_im" in result:
                ...
                logger.info("camera: stored frame %d, will add image to conversation", frame_index)
            ...
            # Other tools (including the camera tool's text answer)
