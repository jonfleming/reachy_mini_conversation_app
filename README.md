---
title: Reachy Mini Conversation (Fleming)
emoji: 🎤
colorFrom: red
colorTo: blue
sdk: static
...
reachy-mini-conversation-fleming --no-camera

# Launch with the minimal web UI for personality/mic/settings control
reachy-mini-conversation-fleming --ui
```
...
> [!NOTE]
> `remember`/`forget` facts are stored in `memory.v1.json` inside the app's instance data directory (`~/.local/share/reachy_mini_conversation_fleming/` by default, or the instance path used by the desktop launcher). `forget` only removes facts matched by query. To reset all remembered facts, delete this file.
...
Create one Python module per tool, with the file name matching the tool's unique `name`. See [`idle_do_nothing.py`](src/reachy_mini_conversation_fleming/tools/idle_do_nothing.py) for a minimal implementation.
...
Profile directories are data-only. Python tool implementations belong in `src/reachy_mini_conversation_fleming/tools/`, or in `REACHY_MINI_EXTERNAL_TOOLS_DIRECTORY` for external tools. Each enabled tool ID must resolve to a shared tool, an external tool, or a tool from an installed Hugging Face Space.
...
To create a locked variant of the app that cannot switch profiles, edit `src/reachy_mini_conversation_fleming/config.py` and set the `LOCKED_PROFILE` constant to the desired profile name:
```python
...
- Core tools are under `src/reachy_mini_conversation_fleming/tools/`.
...
reachy-mini-conversation-fleming tool-spaces add <owner/space-name>

# enable in a specific profile
reachy-mini-conversation-fleming tool-spaces add <owner/space-name> --profile NAME

# install without enabling
reachy-mini-conversation-fleming tool-spaces add <owner/space-name> --install-only

# list installed spaces
reachy-mini-conversation-fleming tool-spaces list

# remove an installed space
reachy-mini-conversation-fleming tool-spaces remove owner/space-name
```
...
```bash
reachy-mini-conversation-fleming --robot-name <name>
```
